...
-- Combine all peak names with all river names, so that the last letter of each peak name is the same as the first letter of its corresponding river name. Display the peak names, river names, and the obtained mix (mix should be in lowercase). Sort the results by the obtained mix. Submit your query statements as Prepare DB & run queries.
*/
SELECT p.PeakName, r.RiverName, LOWER(CONCAT(PeakName, SUBSTRING(RiverName, 2, LEN(RiverName)))) as Mix FROM Rivers r JOIN Peaks p
...
*/
SELECT Username, SUBSTRING(Email, CHARINDEX('@', Email) + 1, LEN(Email)) [Email Provider] FROM Users
...
SELECT ProductName,
       ...
       DATEADD(MONTH, 1, OrderDate) [Deliver Due] FROM Orders
...
SELECT Name, DATEDIFF(YEAR, Birthdate, GETDATE()) [Age in Years], 
       DATEDIFF(YEAR, Birthdate, GETDATE()) * 12 + MONTH(GETDATE()) [Age in Months],
       DATEDIFF(YEAR, Birthdate, GETDATE()) * 12 + MONTH(GETDATE()) + DATEDIFF(DAY, Birthdate, GETDATE()) [Age in Days],
       DATEDIFF(YEAR, Birthdate, GETDATE()) * 12 + MONTH(GETDATE()) + DATEDIFF(DAY, Birthdate, GETDATE()) * 60 * 12 + DATEDIFF(MINUTE, Birthdate, GETDATE()) [Age in Minutes] FROM People
