...
-- You now know how to create database using the GUI of the SSMS. Now it’s time to create it using SQL queries. 
   In that task (and the several following it) you will be required to create the database from the previous 
   exercise using only SQL queries. Firstly, just create new database named Minions.
*/
...
VALUES 
	(1024, 190, 14, 'f', '1991/04/14', 'Strange bio!'),
...
CREATE TABLE Users 
(
	Id BIGINT CONSTRAINT PK_Users PRIMARY KEY IDENTITY CHECK (Id <	9223372036854775808 - 1),
...
VALUES 
	('Elvis', '123321', NULL, '2017/09/21 22:43:14', 0),
...
-- Using SQL queries modify table Users from the previous task. First remove current primary key then create 
   new primary key that would be the combination of fields Id and Username.
*/
ALTER TABLE Users
...
-- Using SQL queries modify table Users. Add check constraint to ensure that the values in the 
...
-- Using SQL queries modify table Users. Remove Username field from the primary key so only the 
   field Id would be primary key. Now add unique constraint to the Username field to ensure that the values 
   there are at least 3 symbols long.
*/
ALTER TABLE Users
DROP CONSTRAINT PK_Users
...
-- Using SQL queries create Movies database with the following entities:
	...
	Make sure the columns that are present in 2 tables would be of the same data type. Consider which fields are 
	always required and which are optional.
*/
...
VALUES 
	('First category name', 'First note'),
...
VALUES 
	('First title', 1, GETDATE(), '11:24:44', 1, 1, 10, 'First note'),
...
VALUES 
	(1 , 1 , 1 ,54 ,10 , 110, 100, GETDATE(), GETDATE() + 1, 1, 1.4, 44, 1, 'Just note'),
...
-- Using SQL queries create Hotel database with the following entities:
    ...
    • Payments (Id, EmployeeId, PaymentDate, AccountNumber, FirstDateOccupied, LastDateOccupied, TotalDays, AmountCharged, 
...
VALUES 
	(1, 'Non Type1', 'Non Type1', 1.1, 0, NULL),
...
VALUES 
	(1, GETDATE(), 1, GETDATE(), GETDATE() + 1, 2, 0.2, 0.1, 3.3, 3.3, 'Note first'),
...
-- Now create bigger database called SoftUni. You will use same database in the future tasks. It should hold information about:
   ...
   Id columns are auto incremented starting from 1 and increased by 1 (1, 2, 3, 4…). Make sure you use appropriate data types 
   for each column. Add primary and foreign keys as constraints for each table. 
...
-- Backup the database SoftUni from the previous tasks into a file named “softuni-backup.bak”. 
   Delete your database from SQL Server Management Studio. Then restore the database from the created backup.
*/
...
-- Use the SoftUni database and first select all records from the Towns, then from Departments and finally from 
   Employees table. Use SQL queries and submit them to Judge at once.
*/
SELECT * FROM Towns
