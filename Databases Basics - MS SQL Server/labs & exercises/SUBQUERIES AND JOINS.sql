...
FROM 
	Employees e
JOIN 
	Departments d ON (e.DepartmentID = d.DepartmentID AND e.HireDate > '1/1/1999' AND d.[Name] IN ('Sales', 'Finance'))
ORDER BY 
	e.HireDate
...
WHERE 
	p.StartDate >= '8/13/2002' AND p.EndDate IS NULL
ORDER BY 
	e.EmployeeID
...
FROM 
	Peaks p
JOIN 
	MountainsCountries mc ON p.MountainId = mc.MountainId
JOIN 
...
FROM 
	Countries c
JOIN 
	Continents co ON c.ContinentCode = co.ContinentCode
LEFT JOIN 
	CountriesRivers cr ON c.CountryCode = cr.CountryCode
LEFT JOIN 
	Rivers r ON cr.RiverId = r.Id
WHERE 
...
FULL JOIN 
	Peaks p ON mc.MountainId = p.MountainId
JOIN 
	CountriesRivers cr ON c.CountryCode = cr.CountryCode
FULL JOIN 
	Rivers r ON cr.RiverId = r.Id
...
-- For each country, find the name and elevation of the highest peak, along with its mountain. 
   When no peaks are available in some country, display elevation 0, "(no highest peak)" as peak name and "(no mountain)" 
   as mountain name. When multiple peaks in some country have the same elevation, display all of them. 
